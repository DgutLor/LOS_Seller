...
//        }
        ...
        getUserDataOnNext = new SubscriberOnNextListener<List<User>>() {
            @Override
        ...
        mBt_login.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                SetAliasAndTagUtil at=new SetAliasAndTagUtil(RegisterAndLoginActivity.this);
                at.setAlias(userName);
                /**=================     调用SDk登陆接口    =================*/
                JMessageClient.login(userName, password, new BasicCallback() {
                    ...
                    public void gotResult(int responseCode, String LoginDesc) {
                        if (responseCode == 0) {
                            ...
                            Toast.makeText(getApplicationContext(), "SDk登陆成功", Toast.LENGTH_SHORT).show();
                            Log.i("MainActivity", "JMessageClient.login" + ", responseCode = " + responseCode + " ; LoginDesc = " + LoginDesc);
                            HttpMethods.getInstance().getUserData(new ProgressSubscriber(getUserDataOnNext, RegisterAndLoginActivity.this,true), userName, password);

                        } else {
                            mProgressDialog.dismiss();
                            Toast.makeText(getApplicationContext(), "SDk登陆失败", Toast.LENGTH_SHORT).show();
                            Log.i("MainActivity", "JMessageClient.login" + ", responseCode = " + responseCode + " ; LoginDesc = " + LoginDesc);
                        }
                    }
                });
...
/**=================     是否设置测试环境    =================*/
    ...
    private void initView() {
        ...
        mEd_userName.setText("13172837508");
        mEd_password = (EditText) findViewById(R.id.ed_login_password);
        mEd_password.setText("hello110");
